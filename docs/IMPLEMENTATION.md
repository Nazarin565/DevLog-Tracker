...
- Mock must be _realistic_ (return schema-valid output) so the real parsing path is tested.
...
- This is the infrastructure that makes agents _extensible_: adding C/D later = one registration.
...
- The agent must consider priority + age + status, _not just rank order_ → that's the "agentic" part (multiple steps + decision).
...
- The reasoning panel makes agenticity _visible_ to the user.
...
- Log in AGENT_LOG _as you go_: what the agent generated, what was edited manually and why.
...
- Almost all DevLog state is _server state_. TanStack Query manages exactly that: cache, invalidation, loading/error out of the box.
...
---Request URL
http://localhost:4000/api/tasks?sortBy=createdAt&order=desc
Referrer Policy
...
- Keep AGENT_LOG _as you go_, not at the end.
